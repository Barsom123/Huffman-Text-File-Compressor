...
void getfilewithextenstion(char *filename, char *outputfilename, const char *newextension)
{
...
public:
    ...
    void getandsavedcodes(char *inputfile, char *outs[256] = {0})
    {
        ...
        getfilewithextenstion(inputfile, codesFilename, ".cod");
...
private:
    char *codes[256] = {0};
    void getcodes(HuffmanNode *root, char *code)
    {
        if (root == NULL)
        {
            return;
        ...
        strcat(leftCode, "0");
        strcat(rightCode, "1");
        getcodes(root->left, leftCode);
        getcodes(root->right, rightCode);
    }
    /*
    void getcodes(HuffmanNode *root, char *code, int index = 0)
    {
        if (root == NULL)
        {
            return;
        ...
        code[index] = '0';
        getcodes(root->left, code, index++);
        code[index] = '1';
        getcodes(root->right, code, index++);
    }
    */
};
...
void compressFile(char *inputFilename, char *outputFilename, HuffmanTree &ht)
{
    ...
    unsigned char buffer = 0;   // to store 8 bit before writing it as a byte in file
    ...
    for (unsigned char i = 0; i < extlen; i++)
    {
        fputc(extension[i], outputFile);
    }
...
HuffmanNode *buildHuffmanTreeFromCodes(char *codes[256])
{
...
void decompressFile(const char *inputFilename)
{
    ...
    getfilewithextenstion((char *)inputFilename, codeFilename, ".cod");
    readcodesfromfile(codeFilename, codes);

    // 3. build huffman tree from codes
    HuffmanNode *root = buildHuffmanTreeFromCodes(codes);
    HuffmanNode *current = root;
    ...
    for (int lastByteIndex = 0; lastByteIndex < dataSize; lastByteIndex++)
    {
        fread(&byte, 1, 1, inputFile);
        int bitsToRead = 8;
        if (lastByteIndex == dataSize - 1)
...
int main(int argc, char *argv[])
{
    ...
    if (!strcmp(argv[1], "-c"))
    {
        ...
        getfilewithextenstion(argv[2], ouputfilename, ".com");
        printf("Compressing %s into %s...\n", argv[2], ouputfilename);
        HuffmanTree *ht = new HuffmanTree();
        compressFile(argv[2], ouputfilename, *ht);
        delete ht;
